...
class ConditionalDenoiseModel(nn.Module):
    ...
    Args:
        ...
        cond_dim (int): Dimension of the condition embedding space.

    Input:
        x (torch.Tensor): Tensor of shape (batch_size, n_channels, height, width)
        t (torch.Tensor): Tensor of shape (batch_size,)
        c (torch.Tensor or None): Tensor of shape (batch_size, cond_dim) or None
    ...
    def __init__(self, n_channels=1, n_classes=1, time_dim=64, cond_dim=2):
        ...
        self.up1 = Up(512, 256)  # (b, 512 + 512, h/8, w/8) to (b, 256, h/4, w/4) skip connection
    ...
    def forward(self, x, t, c=None):
        ...
        # Embed condition if provided
        if c is not None:
            c_embed = self.cond_embed(c).unsqueeze(-1).unsqueeze(-1)  # (batch_size, 512) to (batch_size, 512, 1, 1)
        else:
            c_embed = 0
        ...
        # Add time and condition embeddings to the bottleneck
        x4 = x4 + t_embed + c_embed
