...
class ResBlock(EmbedBlock):
    ...
    """
    def __init__(self, in_ch:int, out_ch:int, tdim:int, cdim:int, num_groups:int, droprate:float, cond_type:str):
        super().__init__()
        self.in_ch = in_ch
        self.out_ch = out_ch
        self.tdim = tdim
    ...
    def __init__(self, in_ch=1, mod_ch=64, out_ch=1, ch_mul=[1,2,4], num_res_blocks=1, cdim=2, use_conv=True, num_groups=16, droprate=0.0, cond_type='add', use_attn=False, dtype=torch.float32):
        ...
        for i, mul in enumerate(self.ch_mul):
            ...
            for _ in range(num_res_blocks):
                layers = [ResBlock(now_ch, nxt_ch, tdim, tdim, self.num_groups, self.droprate, self.cond_type)]
                if self.use_attn:
        ...
        if use_attn:
            ...
            ResBlock(now_ch, now_ch, tdim, tdim, self.num_groups, self.droprate, self.cond_type),
            AttnBlock(now_ch),
            ResBlock(now_ch, now_ch, tdim, tdim, self.num_groups, self.droprate, self.cond_type),
        )
        else:
        ...
        for i, mul in list(enumerate(self.ch_mul))[::-1]:
            ...
            for j in range(num_res_blocks + 1):
                layers = [ResBlock(now_ch + chs.pop(), nxt_ch, tdim, tdim, self.num_groups, self.droprate, self.cond_type)]
                if self.use_attn:
                    layers.append(AttnBlock(nxt_ch))
                now_ch = nxt_ch
