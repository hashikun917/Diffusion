...
def save_anticausal_predictor_checkpoint(predictor, optimizer, epoch, filename='checkpoint.pth'):
    checkpoint = {
...
def save_anticausal_predictor_results(loss_dict, result_dir):
    ...
    for attr, losses in loss_dict.items():
        df = pd.DataFrame({'loss': losses})
        df.to_csv(os.path.join(result_dir, f'{attr}_loss.csv'), index=False)
...
# def load_anticausal_predictor_checkpoint(predictors, optimizers, checkpoint_dir):
    # for attr, predictor in predictors.items():
    #     checkpoint_path = os.path.join(checkpoint_dir, f'{}')
        ### todo:実装途中。チェックポイントをロードするモジュール ###
