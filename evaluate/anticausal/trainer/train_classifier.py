...
from evaluate.anticausal.trainer.utils import get_attribute_ids
...
from evaluate.anticausal.trainer.utils import generate_checkpoint_callback, generate_early_stopping_callback, generate_ema_callback
from torchvision.transforms import RandomHorizontalFlip


dataclass_mapping = {
    "morphomnist": MorphoMNISTLikeForClassifier,
    #"celeba": Celeba,
    #"adni": ADNI
}
...
def train_classifier(classifier, attr, train_set, val_set, config, default_root_dir, weights=None):
    ...
    callbacks = [
        generate_checkpoint_callback(attr + "_classifier", config["ckpt_path"], monitor="val_loss", mode=mode),
        generate_early_stopping_callback(patience=config["patience"], monitor="val_loss", mode=mode, min_delta=1e-5)
    ]
...
if __name__ == "__main__":
    ...
    if dataset == 'morphomnist':
        data = dataclass_mapping[dataset](attribute_size=attribute_size, split="train", normalize_=True, data_dir=data_dir)
