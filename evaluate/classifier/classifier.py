...
class Classifier(pl.LightningModule):
    def __init__(self, attr, in_shape = (1, 28, 28), width=8, num_outputs = 1, context_dim = 0 , lr=1e-3):
        super().__init__()
        self.variable = attr
        ...
        self.lr = lr
        self.variables = {"thickness":0, "intensity":1, "slant":2, "width":3}
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.variable == "thickness":  #condition on intensity when training the thickness classifier
            ...
            cond = torch.cat([
                attrs_[:,self.variables["intensity"]].view(-1, 1),
                attrs_[:,self.variables["slant"]].view(-1, 1),
                attrs_[:,self.variables["width"]].view(-1, 1)
            ], dim=1)
            y_hat = self(x , cond)
