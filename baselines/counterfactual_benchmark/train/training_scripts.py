...
from dataset.transforms import SelectParentAttributesTransform
from baselines.counterfactual_benchmark.models.utils import generate_checkpoint_callback, generate_early_stopping_callback, generate_ema_callback
...
def train_flow(flow, config, data_class, graph_structure, attribute_size, checkpoint_dir, **kwargs):
    ...
    # todo: remove suffix（末尾の文字を削除する）メソッドはpython 3.9から追加されたもののため、アップデートが必要
    flow_name_without_suffix = flow.name[:-5] if flow.name.endswith('_flow') else flow.name
    transform = SelectParentAttributesTransform(flow_name_without_suffix, attribute_size, graph_structure)
