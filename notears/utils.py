...
def simulate_nonlinear_sem2(B, n, sem_type, noise_scale=None):
...
def simulate_nonlinear_sem_with_noise(W, gamma, n, sem_type, noise_scale=None):
    """Simulate samples from non-linear SEM with specified type of noise.
...
def count_accuracy(B_true, B_est, check_dag=True):
    ...
    if (B_est == -1).any():  # cpdag
        ...
        if check_dag and not is_dag(B_est):
...
def plot_weight_matrix(W_true, W_est):
